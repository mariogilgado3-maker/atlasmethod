...
function StoreProvider({ children }) {
  ...
  const actions = React.useMemo(() => ({
    ...
    // Hydrate store from Supabase cloud data; local wins when cloud is empty
    loadFromCloud: (cloudData) => setState(s => ({
      ...s,
      gems: cloudData.gems || s.gems,
      sessions: cloudData.sessions || s.sessions,
      log: cloudData.log?.length > 0 ? cloudData.log : s.log,
      achievements: cloudData.achievements?.length > 0 ? cloudData.achievements : s.achievements,
      reading: cloudData.reading?.completed?.length > 0 ? cloudData.reading : s.reading,
      store: cloudData.store?.owned?.length > 0 ? cloudData.store : s.store,
      plan: cloudData.plan !== undefined ? cloudData.plan : s.plan,
    })),
